...
/**
 * Creates the initial state for a single rope.
 */
...
/**
 * Custom hook to manage the physics simulation for all ropes.
 */
function useRopesPhysics(specs) {
  ...
  const startDrag = useCallback((ropeIndex, pointerX, pointerY) => {
    ...
    draggingState.current.offset.set(
      endpoint.x - pointerX,
      endpoint.y - pointerY
    );
  }, []);
  ...
  const step = useCallback((dt) => {
    ropesRef.current.forEach((rope, ropeIndex) => {
      ...
      for (let i = 1; i < particles.length; i++) {
        ...
        p.pos.x +=
          Math.sin(i * 0.1 + performance.now() * 0.001) * PENDULUM_SWAY;
        p.prev.copy(tempPos);
      }
      ...
      if (draggingState.current.index === ropeIndex) {
        ...
        if (totalDist > maxLen) {
          ...
          endpoint.pos
            .copy(anchor.pos)
            .add(totalDelta.multiplyScalar(stretchRatio));
        }
      }
    });
  }, []);
...
/**
 * Renders the ropes and syncs the DOM icons to the rope endpoints.
 */
function RopesRenderer({ anchors }) {
  ...
  const { ropesRef, startDrag, moveDrag, endDrag, step } =
    useRopesPhysics(specs);

  // This single, robust useEffect handles finding DOM nodes, styling them,
  // and attaching event listeners in a safe, isolated way.
  useEffect(() => {
    ...
    const calculateSpecs = () => {
      const newSpecs = anchors.map((pageIndex, i) => {
        ...
        const rect = el
          ...
          : { left: 50 + i * 50, width: 40, top: 50 };
        const anchorX = Math.round(rect.left + rect.width / 2);
        const anchorY =
          pageIndex * window.innerHeight + rect.top + rect.height / 2;
    ...
    const onPointerMove = (e) => {
      ...
      if (!pointerState.hasMoved) {
        ...
        if (dist > DRAG_START_THRESHOLD) {
          ...
          startDrag(
            pointerState.candidateIndex,
            pointerState.startX,
            pointerState.startY + window.scrollY
          );
          pointerState.hasMoved = true;
        }
      }
      if (pointerState.hasMoved) {
        moveDrag(x, y + window.scrollY);
      }
    };
    ...
    const onIconPointerDown = (e, iconIndex) => {
      ...
      pointerState.holdTimer = setTimeout(() => {
        startDrag(iconIndex, x, y + window.scrollY);
        pointerState.hasMoved = true;
      }, DRAG_HOLD_DURATION);
  ...
  useFrame((_, delta) => {
    ...
    ropesRef.current.forEach((rope, i) => {
      ...
      const points = rope.particles.map(
        (p) =>
          new THREE.Vector3(
            p.pos.x - size.width / 2,
            -(p.pos.y - window.scrollY - size.height / 2),
            0
          )
      );
      ...
      if (icon) {
        ...
        icon.style.transform = `translate3d(${Math.round(
          endpoint.x - w / 2
        )}px, ${Math.round(endpoint.y - window.scrollY - h / 2)}px, 0)`;
      }
    });
  });
...
/**
 * The main export component that sets up the full-screen canvas.
 */
export default function HangingSocials({ anchors }) {
  return (
    <div
      className="hanging-socials-canvas"
      style={{
        position: "fixed",
        top: 0,
        left: 0,
